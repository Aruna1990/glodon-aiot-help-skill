#!/usr/bin/env python3
"""
学习 @glodon-aiot/bot-client-ui 文档（旧版前端 SDK）

这是广联达行业 AI 平台旧版的前端集成方式，已被 Coze Studio 取代。
用于了解历史项目和迁移指南。

用法：
    python3 learn_bot_client_ui.py
"""
...
OUTPUT_FILE = KNOWLEDGE_DIR / "bot_client_ui.md"
...
def fetch_npm_info():
    ...
    url = "https://registry.npmjs.org/@glodon-aiot/bot-client-ui"
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            return {
                ...
                'description': data.get('description', '广联达 AI 平台旧版前端 Bot 客户端 UI SDK'),
        ...
        return {
            'version': 'unknown',
            'description': '广联达 AI 平台旧版前端 Bot 客户端 UI SDK',
...
def generate_doc():
    ...
    doc = f"""# @glodon-aiot/bot-client-ui 文档（旧版）

> **⚠️ 已废弃** - 此 SDK 已被 **Coze Studio** 取代，新项目请使用 `@glodon-aiot/chat-app-sdk`  
> **版本**: {npm_info['version']} | **最后更新**: {now} | **来源**: NPM Registry + 广联达官方文档
...
_本文档由 glodon-ai-help 技能自动生成，最后更新：{now}_
...
def main():
    print("📚 开始学习 @glodon-aiot/bot-client-ui 文档（旧版 SDK）...")
    ...
    state['bot_client_ui'] = {
        ...
        'source': 'npm_registry',
        'status': 'deprecated',
        'replacement': '@glodon-aiot/chat-app-sdk'
    }
